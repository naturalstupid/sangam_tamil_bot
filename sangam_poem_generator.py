...
model_name =  "agananuru_poems" # "agananuru_poems"
saved_model_file = model_weights_folder+model_name+"_final.h5"
checkpoint_suffix = '_chpt.h5'
print("reading ",sangam_csv_folder+"sangam_poems.csv")
df = pd.read_csv(sangam_csv_folder+"sangam_poems.csv")
df= df.sample(frac=1).reset_index(drop=True)
poem_data = df[df.poem_type.str.contains("அகநானூறு")] # df
epoch_max = 30
#Data Preparation
...
def create_model(model_name, df, path_to_save_model, callbacks, build_model_func, checkpoint_dir, EPOCHS=epoch_max):
    ...
    if os.path.exists(saved_model_file):
        print("Reading saved model from",saved_model_file)
    ...
    print('New model to be built and trained since',saved_model_file,'does not exist')
    model = build_model_func(vocab_size, embedding_dim, rnn_units, BATCH_SIZE)
    ...
    test_model.load_weights(checkpoint_dir)
    #test_model.build(tf.TensorShape([1, None]))
    
    return model, test_model, char2idx, idx2char
def get_callbacks(checkpoint_dir):
    # Name of the checkpoint files
    checkpoint_prefix = checkpoint_dir# os.path.join(checkpoint_dir, "ckpt")
...
def create_model_for(model_name, data_df):
    path_to_store = saved_model_file
    checkpoint_dir = model_weights_folder+model_name+checkpoint_suffix
    ...
    model, test_model, char2idx, idx2char = create_model(
        ...
        build_model,
        checkpoint_dir)
    results_df = evaluate_model(test_model, model_name, char2idx, idx2char, debug=False)
    #results_df.to_csv(path_to_store+"result.csv")
    print("Saving model weights in",saved_model_file)
    model.save_weights(saved_model_file)
    return results_df
results_for_model = create_model_for(model_name, poem_data)
print(results_for_model)
